...
def get_metric_data():
    ...
    clock, model, type_, socket, corepersocket = server_info
    ...
    metric_query = {
        ...
        'saps': "SELECT sd_bench, hana_prod FROM saps WHERE id_server = (SELECT id_server FROM server WHERE name = %s AND cores = %s)",
...
def save_configurations():
    ...
    for config in configurations:
        cursor.execute(
            ...
            (generation, server, cores, clock, model, type, socket, corepersocket, st, smt2, smt4, smt8, sd_bench_saps, hana_prod_saps, specrate2017_int_peak, specrate2017_int_basek, cpw, percentage)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''',
            (config.get('generation'), config.get('server'), config.get('cores'),
             config.get('clock'), config.get('model'), config.get('type'),
             config.get('socket'), config.get('corepersocket'),
